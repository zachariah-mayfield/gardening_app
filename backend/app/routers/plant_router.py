...
async def add_plant(
    ...
    existing = db.query(models.Plant).filter(
        models.Plant.name.ilike(plant.name.strip())
    ).first()
...
async def update_plant_by_id(
    ...
    name_conflict = db.query(models.Plant).filter(
    ...
    if name_conflict:
        ...
        raise HTTPException(
            status_code=400,
            detail="Another plant with this name already exists"
        )
...
async def update_plant_by_name(
    ...
    db_plant = db.query(models.Plant).filter(
        models.Plant.name.ilike(plant_name.strip())
    ).first()
