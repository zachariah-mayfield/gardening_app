...
def test_create_plant_success():
    ...
    response = client.post("/api/v1/plants", json={"name": "TestPlant1", "description": "desc1"})
...
def test_create_plant_duplicate_name():
    ...
    client.post("/api/v1/plants", json={"name": "TestPlantDup", "description": "desc"})
    response = client.post("/api/v1/plants", json={"name": "TestPlantDup", "description": "desc again"})
...
def test_get_plants():
    ...
    client.post("/api/v1/plants", json={"name": "TestPlantGet", "description": "desc"})
    response = client.get("/api/v1/plants")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)  # The response should be a list
    assert any(plant["name"] == "TestPlantGet" for plant in data)

def test_update_plant_by_id_success():
    # Test updating a plant by its ID (PUT request)
    create = client.post("/api/v1/plants", json={"name": "TestPlantUpdateID", "description": "desc"})
    plant_id = create.json()["id"]
    response = client.put(f"/api/v1/plants/id/{plant_id}", json={"name": "UpdatedNameID", "description": "newdesc"})
    assert response.status_code == 200
    data = response.json()
...
def test_update_plant_by_name_success():
    ...
    client.post("/api/v1/plants", json={"name": "TestPlantUpdateName", "description": "desc"})
    response = client.put("/api/v1/plants/name/TestPlantUpdateName", json={"name": "UpdatedNameByName", "description": "desc2"})
    assert response.status_code == 200
    data = response.json()
...
def test_update_nonexistent_plant_by_id():
    ...
    response = client.put("/api/v1/plants/id/999999", json={"name": "NoPlant", "description": "none"})
...
def test_update_nonexistent_plant_by_name():
    ...
    response = client.put("/api/v1/plants/name/NoSuchPlant", json={"name": "NoPlant", "description": "none"})
    assert response.status_code == 404  # Should return 404 Not Found

def test_delete_plant_by_id():
    # Test deleting a plant by its ID (DELETE request)
    response = client.post("/api/v1/plants", json={"name": "TestDeleteID", "description": "desc"})
...
def test_delete_plant_by_name():
    ...
    client.post("/api/v1/plants", json={"name": "TestDeleteName", "description": "desc"})
