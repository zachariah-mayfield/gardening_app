...
describe('App Component Tests', () => {
    ...
    beforeEach(() => {
        ...
        api.updatePlantByName.mockImplementation((name, plant) => Promise.resolve({ ...plant, name }));
    ...
    test('deletes plant by name when delete button is clicked and no ID', async () => {
        ...
        render(<App />);
        await waitFor(() => expect(screen.getByText('Nameless Plant')).toBeInTheDocument());
        fireEvent.click(screen.getByText('Delete'));
        await waitFor(() => expect(api.deletePlantByName).toHaveBeenCalledWith('Nameless Plant'));
    });

    test('adds a new plant successfully', async () => {
        render(<App />);
        await waitFor(() => expect(screen.getByText('Rose')).toBeInTheDocument());
        fireEvent.change(screen.getByLabelText('Plant Name:'), { target: { value: 'Basil' } });
        fireEvent.change(screen.getByLabelText('Description:'), { target: { value: 'Herb' } });
        fireEvent.click(screen.getByText('Add Plant'));
        await waitFor(() => expect(api.addPlant).toHaveBeenCalledWith({ name: 'Basil', description: 'Herb' }));
    });
    ...
    test('shows error when updating non-existent plant', async () => {
        api.updatePlantById.mockRejectedValueOnce(new Error('Failed to update plant'));
        render(<App />);
        await waitFor(() => expect(screen.getByText('Rose')).toBeInTheDocument());
        fireEvent.click(screen.getAllByText('Edit')[0]);
        fireEvent.change(screen.getByDisplayValue('Rose'), { target: { value: 'Ghost Plant' } });
        fireEvent.click(screen.getByText('Update Plant'));
        await waitFor(() => expect(screen.getByText(/failed to update plant/i)).toBeInTheDocument());
    });

    test('shows error when deleting non-existent plant by ID', async () => {
        api.deletePlantById.mockRejectedValueOnce(new Error('Failed to delete plant'));
        render(<App />);
        await waitFor(() => expect(screen.getByText('Rose')).toBeInTheDocument());
        fireEvent.click(screen.getAllByText('Delete')[0]);
    ...
    test('shows error when deleting non-existent plant by name', async () => {
        ...
        api.deletePlantByName.mockRejectedValueOnce(new Error('Failed to delete plant'));
        render(<App />);
        await waitFor(() => expect(screen.getByText('Nameless Plant')).toBeInTheDocument());
        fireEvent.click(screen.getByText('Delete'));
        await waitFor(() => expect(screen.getByText(/failed to delete plant/i)).toBeInTheDocument());
    ...
    test('UI disables delete/edit during loading', async () => {
        ...
        let resolveFetch;
        api.fetchPlants.mockImplementation(() => new Promise(res => { resolveFetch = res; }));
        render(<App />);
